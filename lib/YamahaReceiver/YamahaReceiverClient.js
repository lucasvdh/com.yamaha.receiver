...
class YamahaReceiverClient {
    ...
    constructor(ipAddress) {
        ...
        this.playInfoTransformers = {
            [this.getAirPlayZone()]: function (result) {
                let playInfo = defaultPlayInfo();
                console.log('airplay result:' + JSON.stringify(result, undefined, 2));
                let playInfoResult = result['YAMAHA_AV'][this.getAirPlayZone()][0]['Play_Info'][0],
            ...
            [this.getSpotifyZone()]: function (result) {
                let playInfo = defaultPlayInfo();
                console.log('spotify result:' + JSON.stringify(result, undefined, 2));
                let playInfoResult = result['YAMAHA_AV'][this.getSpotifyZone()][0]['Play_Info'][0],
                    availabilityResult = playInfoResult['Feature_Availability'][0],
                    playbackInfoResult = playInfoResult['Playback_Info'][0],
                    metaInfoResult = playInfoResult['Meta_Info'][0];
                playInfo.available = availabilityResult === 'Ready';
                ...
                playInfo.artist = Entities.decode(metaInfoResult['Artist'][0]);
                playInfo.album = Entities.decode(metaInfoResult['Album'][0]);
                playInfo.track = Entities.decode(metaInfoResult['Track'][0]);
                return playInfo;
            }
        };
    }
