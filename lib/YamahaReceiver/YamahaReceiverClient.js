...
const BasicStatusStateTransformer = require('./Transformers/BasicStatus/StateTransformer');
const PlayInfoAirPlayTransformer = require('./Transformers/PlayInfo/AirPlayTransformer');
const PlayInfoSpotifyTransformer = require('./Transformers/PlayInfo/SpotifyTransformer');
const PlayInfoUSBTransformer = require('./Transformers/PlayInfo/USBTransformer');
...
class YamahaReceiverClient {
        ...
        return this.getZone().then(zone => {
            if (this.hasPlayInfo(zone)) {
                return this.get(zone, '<Play_Info>GetParam</Play_Info>').then(xmlResponse => {
                    return this.getPlayInfoTransformerByZone(zone).transform(xmlResponse);
                });
            } else {
    ...
    getState() {
        return this.request('GET', this.getMainZone(), '<Basic_Status>GetParam</Basic_Status>')
            .then(xmlResponse => {
                return new BasicStatusStateTransformer().transform(xmlResponse);
            });
    }
    ...
    getPlayInfoTransformerByZone(zone) {
        if (zone === this.getAirPlayZone()) {
            return new PlayInfoAirPlayTransformer();
        } else if (zone === this.getIPodUSBZone()) {
            return new PlayInfoIpodUSBTransformer();
        } else if (zone === this.getSpotifyZone()) {
            return new PlayInfoSpotifyTransformer();
        } else if (zone === this.getNetRadioZone()) {
            return new PlayInfoNetRadioTransformer();
        } else if (zone === this.getServerZone()) {
            return new PlayInfoServerTransformer();
        } else if (zone === this.getJukeZone()) {
            return new PlayInfoJukeTransformer();
        } else if (zone === this.getTunerZone()) {
            return new PlayInfoTunerTransformer();
        } else if (zone === this.getUSBZone()) {
            return new PlayInfoUSBTransformer();
        } else if (zone === this.getMusicCastLinkZone()) {
            return new PlayInfoMusicCastLinkTransformer();
        }

        throw new Error('Could not find PlayInfoTransformer for zone "' + zone + '"');
    }
}
